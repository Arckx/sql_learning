...
-- List all cities where offices are located.
select city 
from offices;

-- Retrieve all customers based in the USA.
select customerName, country
from customers
where country = 'USA';

-- Show the names and emails of all employees.
select firstName, lastName, email
from employees;

-- Find all products whose name starts with “1969”.
select productName
from products
where productName like '1969%';

-- Get all products with quantities in stock less than 100.
select productName, quantityInStock
from products
where quantityInStock < 100;

-- Show all customers whose credit limit is more than $100,000.
select customerName, creditLimit
from customers
where creditLimit > 100000;

-- Retrieve customers located in cities that start with ‘S’.
select customerName, city
from customers
where city like 'S%';

-- Find all employees who work in office code ‘1’.
...
-- Show all customers who do not have an assigned sales rep.
select customerName, salesRepEmployeeNumber
from customers
where salesRepEmployeeNumber is null;

-- List all customers and their sales representative’s name.
select c.customerName, c.salesRepEmployeeNumber, e.employeeNumber, e.firstName, e.lastName
from customers c inner join employees e
where c.salesRepEmployeeNumber = e.employeeNumber;

-- Show total number of orders per customer.
select c.customerNumber, c.customerName, o.customerNumber, o.orderNumber, count(o.orderNumber) as total_orders
from customers c inner join orders o
where c.customerNumber = o.customerNumber
group by c.customerName;

-- Get total quantity of items sold per product.
select o.orderNumber, od.orderNumber, od.productCode, od.quantityOrdered, count(od.quantityOrdered) as total_qty
from orders o inner join orderdetails od
where o.orderNumber = od.orderNumber
group by od.productCode
order by od.productCode;

-- Show average credit limit by country.
